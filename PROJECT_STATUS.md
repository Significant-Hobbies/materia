...
| 2026-07-03 | **Production deploy** — all 553 cited pages deployed to Cloudflare Pages via `npm run deploy` (guarded `workflow_dispatch`). Live at `materia-6bq.pages.dev`. Desired custom domain `materia.significanthobbies.com` is not yet wired. |
...
Desired custom domain `materia.significanthobbies.com` is not yet configured. Build output: `dist/`.
