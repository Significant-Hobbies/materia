...
**In scope:** interactive 3D layered explorer (per-part mesh clicking; 2D SVG fallback); per-entity content pages (body part,
...
| 2026-07-17 | **Custom domain** — `materia.significanthobbies.com` attached to the existing Cloudflare Pages project and verified live. |
| 2026-07-03 | **Production deploy** — all 553 cited pages deployed to Cloudflare Pages via `npm run deploy` (guarded `workflow_dispatch`). |

## Products

**Live:** `https://materia.significanthobbies.com/` — all **553 static pages** deployed 2026-07-03 via
...
(91 curated + 98 drug stubs + 18 herb stubs) · 60 compounds · 175 studies · 37 sources. **9 anatomy layers** as decimated,
Draco-compressed Z-Anatomy GLBs (~15 MB total) under `public/models/`. **All 24 body parts lead to cited content** (no orphans).
...
- **Every efficacy claim must cite ≥1 study/source** — enforced by a Zod `.refine((c) => c.studies.length + c.sources.length > 0)` on `efficacyClaim`; build fails otherwise.
...
- 2D SVG anatomy art pass — the 3D renderer (`ThreeBody.tsx`) shipped and is the
  active explorer; the 2D `AnatomyBody.tsx` fallback still only draws 13 anterior
  regions. Expanding/posterior 2D art is paused.

### Blocked / Known gaps
- 12 body parts (brain, heart, intestines, kidneys, liver, lower-back, lungs,
  pancreas, skin, stomach, thyroid, upper-back) have no region in the 2D SVG
  fallback (`AnatomyBody.tsx` draws 13 anterior regions) — `npm run checks`
  warns on each. They are still fully clickable in the active 3D explorer
  (`ThreeBody.tsx` maps all 24). Only affects the 2D fallback path.
